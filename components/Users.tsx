    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Manage user access and system roles.</p>
        </div>
        <button
        ...
        <div className="p-4 border-b border-slate-100 flex items-center bg-slate-50/50">
           <div className="relative max-w-sm w-full">
             ...
             <input 
                ...
                className="w-full pl-10 pr-4 py-2 bg-white border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all placeholder:text-slate-400 text-slate-700" 
             />
           </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-slate-50/80 text-slate-500 uppercase text-[11px] font-bold tracking-wider">
              <tr>
                ...
                <th className="px-6 py-4 border-b border-slate-100">Access Role</th>
            ...
            <tbody className="divide-y divide-slate-100 bg-white">
              {loading ? (
                ...
                users.map((user) => (
                  <tr key={user.id} className="group hover:bg-slate-50 transition-colors duration-150">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex justify-end space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => handleUpdateRole(user)} className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors">
                          <Edit2 size={16} />
                        </button>
                ...
                <tr>
                  <td colSpan={4}>
                    <div className="flex flex-col items-center justify-center py-24 text-center">
                      ...
                      <h4 className="text-slate-700 font-semibold mb-1">No users found</h4>
                      <p className="text-slate-400 text-sm max-w-xs mx-auto mb-6">
                        There are no users registered in the system yet. Start by adding a new team member.
                      </p>
                      <button 
                        onClick={() => setIsModalOpen(true)}
          ...
          <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md animate-slide-up ring-1 ring-black/5">
            <div className="flex justify-between items-start mb-6">
              <div>
                ...
                <p className="text-slate-500 text-sm mt-1">Create credentials for a new system user.</p>
              </div>
            ...
            {error && <p className="text-red-600 bg-red-50 border border-red-100 p-3 rounded-lg text-sm mb-5 font-medium">{error}</p>}
            ...
            <form onSubmit={handleCreateUser} className="space-y-5">
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none appearance-none transition-all text-sm cursor-pointer"
                    value={newUser.role}
